...
class Debian(BaseSystemRepository):
    ...
    def generate_initramfs(self, mount_point: str, kver: str) -> None:
        ...
        Note: On Ubuntu/Debian, the kernel package postinst usually triggers
        initramfs generation automatically. This function ensures it exists
        ...
        Raises:
            ...
            RuntimeError: If initramfs generation fails
        """
        if not kver:
            raise ValueError(
        ...
        logger.info(f"Checking initramfs for kernel {kver}...")

        # Check if initramfs already exists (usually created by kernel postinst)
        initramfs_path = f"/boot/initrd.img-{kver}"
        check_exists = exec_chroot(
            f"test -f {initramfs_path} && echo 'EXISTS' || echo 'MISSING'",
            mount_point=mount_point,
            get_output=True,
        ).strip()

        if check_exists == "EXISTS":
            logger.info(f"✓ Initramfs already exists: {initramfs_path}")
            logger.info("Initramfs was generated by kernel package postinst trigger")
        else:
            logger.info(f"Initramfs not found, generating for kernel {kver}...")

            # Generate initramfs using Debian's tool with full PATH
            # -c = create, -k = kernel version
            try:
                exec_chroot(
                    f"PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin update-initramfs -c -k {kver}",
                    mount_point=mount_point,
                )
            except Exception as e:
            ...
            logger.info(f"✓ Initramfs generated: {initramfs_path}")

        # Debian creates: /boot/initrd.img-{kver}
        # pykod expects: /boot/initramfs-linux-{kver}.img
        # Create symlink for compatibility if needed
        pykod_initramfs = f"/boot/initramfs-linux-{kver}.img"
        check_symlink = exec_chroot(
            f"test -L {pykod_initramfs} && echo 'EXISTS' || echo 'MISSING'",
            mount_point=mount_point,
            get_output=True,
        ).strip()

        if check_symlink == "MISSING":
            logger.debug("Creating pykod-compatible initramfs symlink...")
            exec_chroot(
                f"ln -sf {initramfs_path} {pykod_initramfs}",
                mount_point=mount_point,
            )

        # Verify the initramfs exists
        verify_cmd = exec_chroot(
            f"test -f {initramfs_path} && echo 'OK' || echo 'MISSING'",
            mount_point=mount_point,
            get_output=True,
        ).strip()
        ...
        if verify_cmd == "MISSING":
            raise RuntimeError(
                f"Initramfs file {initramfs_path} not found after generation. "
                f"Kernel package postinst may have failed."
            )
    ...
    def get_sudo_group(self) -> str:
        """Get the sudo group name for Debian/Ubuntu.
    ...
    def configure_locale(self, mount_point: str, locale_config) -> None:
        """Configure locale for Debian/Ubuntu.

        Uses /etc/default/locale for locale settings.
        ...
        logger.info(f"Configuring Debian/Ubuntu locale: {locale_config.default}")
        ...
        try:
            ...
            logger.error("Make sure 'locales' package is installed in base packages")
            raise RuntimeError(f"Failed to generate locales: {e}") from e

        # Set default locale in /etc/default/locale (Debian/Ubuntu-specific)
        ...
        with open_with_dry_run(f"{mount_point}/etc/default/locale", "w") as locale_file:
